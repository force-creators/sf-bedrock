...
(`Apex__c`, `Batch_Size__c`, `Max_Retries__c`), plus the hierarchical custom
setting `Async_Settings__c`, all under `force-app/bedrock/lib/async/objects`.
`Retry_Count__c` (default `0`) tracks how many times the framework has re-run
an item; `Max_Retries__c` (absent/`0` ⇒ auto-retry off) caps framework
auto-retries per job type.
